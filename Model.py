...
class Model(nn.Module):
	...
	def forward_MM(self, adj, image_adj, text_adj, audio_adj=None):
		...
		embedsModalFused = torch.concat([self.uEmbeds, F.normalize(embedsModal[args.user:])])
		...
		for gcn in self.gcnLayers:
			embeds = gcn(adj, svt_embedsLst[-1])
			svt_embedsLst.append(embeds)
		svt_embeds = sum(svt_embedsLst)
		...
		for gcn in self.gcnLayers:
			embeds = gcn(adj, st_embedsLst[-1])
			st_embedsLst.append(embeds)
		st_embeds = sum(st_embedsLst)
		...
		for gcn in self.gcnLayers:
			embeds = gcn(adj, sv_embedsLst[-1])
			sv_embedsLst.append(embeds)
		sv_embeds = sum(sv_embedsLst)
		...
		if audio_adj != None:
			...
			for gcn in self.gcnLayers:
				embeds = gcn(adj, sa_embedsLst[-1])
				sa_embedsLst.append(embeds)
			sa_embeds = sum(sa_embedsLst)
